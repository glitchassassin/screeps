...
export class Route {
  ...
  constructor(creep: Creep, public key: string, public targets: MoveTarget[], public opts?: MoveToOpts) {
  ...
  calculateRoute(creep: Creep) {
    ...
    for (const target of this.targets) {
      ...
      let roomsRoute = Game.map.findRoute(creep.pos.roomName, target.pos.roomName, {
    ...
    if (!bestRoute)
      throw new Error(
        `No valid room path ${creep.pos.roomName} - [${this.targets.map(t => t.pos.roomName).join(',')}]`
      );
  ...
  calculatePathToRoom(creep: Creep, avoidCreeps = false, recalculated = false) {
    ...
    this.calculatePath(
      creep,
      creep.room.find(exit).map(pos => ({ pos, range: 0 })),
      avoidCreeps
    );
  }
...
export function moveTo(
  creep: Creep,
  targetOrTargets: _HasRoomPosition | RoomPosition | MoveTarget | MoveTarget[],
  opts?: MoveToOpts | undefined
) {
  const targets: MoveTarget[] = [];
  if ('range' in targetOrTargets) {
    // MoveTarget
