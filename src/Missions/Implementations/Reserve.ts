  ...
  static minionLogic(mission: ReserveMission, creep: Creep): void {
    if (!mission.data.reserveTarget) {
      ...
      const potentialTargets = harvestMissions.filter(h => {
        ...
        return (
          currentMissions.every(m => sourcePos.roomName !== m.data.reserveTarget || missionExpired(m)) &&
          // Has less than 3000 ticks of reservation
          !(
            Memory.rooms[sourcePos.roomName].reserver === 'LordGreywether' &&
            (Memory.rooms[sourcePos.roomName].reservation ?? 0) >= 3000
          )
        );
      });
      ...
      let bestTargets = potentialTargets.filter(
        h => assignedCreep(h)?.ticksToLive && assignedCreep(h)!.ticksToLive! > CREEP_CLAIM_LIFE_TIME
      );
      if (!bestTargets.length) bestTargets = potentialTargets;

      const closestTarget = bestTargets.reduce((a, b) =>
        (a.data.distance ?? Infinity) < (b.data.distance ?? Infinity) ? a : b
      );
