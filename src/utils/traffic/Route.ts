...
export class Route {
  ...
  constructor(creep: Creep, public pos: RoomPosition, public range: number = 1) {
  ...
  calculateRoute(creep: Creep) {
    ...
    if (creep.pos.roomName !== this.pos.roomName) {
      let roomsRoute = Game.map.findRoute(creep.pos.roomName, this.pos.roomName, {
      ...
      if (roomsRoute === ERR_NO_PATH)
        throw new Error(`No valid room path ${creep.pos.roomName} - ${this.pos.roomName}`);
  ...
  calculatePathToRoom(creep: Creep, avoidCreeps = false, recalculated = false) {
    ...
    if (!nextRoom) {
      ...
      let positionsInRange = calculateNearbyPositions(this.pos, this.range, true).filter(
        pos => isPositionWalkable(pos, true) && pos.x > 0 && pos.x < 49 && pos.y > 0 && pos.y < 49
      );
