import { MineMission } from 'Missions/Implementations/MineMission';
...
export default () => {
  for (let office in Memory.offices) {
    ...
    for (const mission of activeMissions(office)) {
      if (isMission(MineMission)(mission)) mining = true;
      if (isMission(ScienceMission)(mission)) science = true;
    }
    ...
    Game.map.visual.rect(new RoomPosition(0, 40, office), 50 * Math.max(0, Math.min(1, meterProgress)), 10, {
      stroke: 'transparent',
      fill: meterColor
    });
