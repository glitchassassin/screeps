...
const missionsStatsTable = (room: string) => {
    ...
    for (const type in Memory.offices[room].missionResults) {
        ...
        stats.efficiency.avg = (stats.efficiency.sum * 100) / Memory.offices[room].missionResults[type as MissionType]!.length;
            ...
            widgets: [
                {
                    ...
                    widget: Rectangle({ data: Table({
                        config: { headers: ['Mission', 'Surplus CPU (min/avg/max)', 'Surplus Energy (min/avg/max)', 'Efficiency', 'Total Energy'] },
                        data: missionsStats
                    }) })
                }
            ],
