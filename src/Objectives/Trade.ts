...
export class TradeObjective extends Objective {
    budget(office: string, energy: number) {
        if (!roomPlans(office)?.headquarters?.terminal.structure) {
            return {
                cpu: 0,
                spawn: 0,
                energy: 0
        ...
        for (let [resource, [office, amount]] of maxSurpluses) {
            ...
            if (targetOffice && targetAmount && Math.abs(targetAmount) > TERMINAL_SEND_THRESHOLD) {
                ...
                if (transferAmount > TERMINAL_SEND_THRESHOLD) {
                    ...
                    if (result === OK) {
                        this.recordEnergyUsed(office, cost);
                ...
                if (order) {
                    ...
                    if (Game.market.deal(order.id, Math.min(amount, order.amount), office) === OK) {
                        this.recordEnergyUsed(office, cost)
                    }
                }
            }
            ...
            if (!targetOffice && Math.abs(amount) > TERMINAL_SEND_THRESHOLD) {
                ...
                if (order) {
                    ...
                    if (cost < terminalBalance(office, RESOURCE_ENERGY) / 10) {
                        ...
                        if (result === OK) this.recordEnergyUsed(office, cost)
                    }
                }
            }
        }
