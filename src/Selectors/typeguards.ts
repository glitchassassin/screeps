...
export const isPlannedStructure =
  <T extends BuildableStructureConstant = BuildableStructureConstant>(type?: T) =>
  (structure?: PlannedStructure): structure is PlannedStructure<T> =>
    !!structure && (!type || structure.structureType === type);

export const isNotNull = <T>(b: T): b is Exclude<T, null> => b !== null;
