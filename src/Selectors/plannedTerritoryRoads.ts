...
import { adjustedEnergyForPlannedStructure, costForPlannedStructure, plannedStructureNeedsWork } from "./facilitiesWorkToDo";
...
export function franchisesThatNeedRoadWork(office: string) {
    return franchisesByOffice(office)
        .filter(({remote, source, room}) => {
            ...
            plannedFranchiseRoads(office, source).some(s => plannedStructureNeedsWork(s))
        })
        .map(({source}) => source)
}
