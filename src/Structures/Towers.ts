...
export const runTowers = () => {
  for (let office in Memory.offices) {
    ...
    if (!plan?.backfill || !Game.rooms[office]) return;
    ...
    for (let target of targets) {
      const damage = plan.backfill.towers.reduce(
        (sum, t) => sum + towerDamage(t.structure as StructureTower | undefined, target.pos),
        0
      );
