...
import { validateHeadquartersPlan } from '../Stamps/validateHeadquartersPlan';
...
export const planHeadquarters = (roomName: string) => {
  ...
  for (let space of findSpaces(controllerPos, currentRoomPlan)) {
    ...
    if (space.horizontal) {
      new RoomVisual(roomName).rect(space.x, space.y, HQ_UPGRADE_BOTTOM[0].length - 1, HQ_UPGRADE_BOTTOM.length - 1, {
        fill: 'transparent',
        stroke: 'white'
      });
      if (
        new RoomPosition(space.x + ANCHOR_BOTTOM.x, space.y + ANCHOR_BOTTOM.y, controllerPos.roomName).inRangeTo(
          controllerPos,
          3
        )
      ) {
        ...
        new RoomPosition(space.x + ANCHOR_TOP.x, space.y + ANCHOR_TOP.y, controllerPos.roomName).inRangeTo(
          controllerPos,
          3
        )
      ) {
      ...
      new RoomVisual(roomName).rect(space.x, space.y, HQ_UPGRADE_RIGHT[0].length - 1, HQ_UPGRADE_RIGHT.length - 1, {
        fill: 'transparent',
        stroke: 'white'
      });
      if (
        new RoomPosition(space.x + ANCHOR_RIGHT.x, space.y + ANCHOR_RIGHT.y, controllerPos.roomName).inRangeTo(
          controllerPos,
          3
        )
      ) {
        ...
        new RoomPosition(space.x + ANCHOR_LEFT.x, space.y + ANCHOR_LEFT.y, controllerPos.roomName).inRangeTo(
          controllerPos,
          3
        )
      ) {
...
function* findSpaces(controllerPos: RoomPosition, currentRoomPlan: CostMatrix) {
  ...
  for (let yGrid = 0; yGrid < height; yGrid++) {
    ...
    for (let xGrid = 0; xGrid < width; xGrid++) {
      ...
      new RoomVisual(controllerPos.roomName).text(
        `${grid[yGrid][xGrid].x} ${grid[yGrid][xGrid].y}`,
        x + xGrid,
        y + yGrid,
        { font: 0.2 }
      );
