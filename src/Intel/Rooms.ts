...
export const scanRooms = profiler.registerFN(() => {
  ...
  for (let room in Game.rooms) {
    ...
    Memory.rooms[room].exits ??= packPosList(Game.rooms[room].find(FIND_EXIT));
    ...
    if (
      Game.rooms[room].find(FIND_HOSTILE_STRUCTURES, { filter: s => s.structureType === STRUCTURE_INVADER_CORE })
        .length > 0
    ) {
